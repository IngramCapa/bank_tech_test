...
describe Account do
  ...
  describe "#print" do
    let(:print_statement) { double :print_statement }

    before(:each) do
      allow(print_statement).to receive(:print_header) { puts 'date || credit || debit || balance' }
      allow(print_statement).to receive(:print_transactions) { puts "13/01/2012 || || 800.00 || 200.00\n10/01/2012 || 1000.00 || || 1000.00\n" }
    end

    it "prints the statement" do
      expect { subject.print(print_statement) }.to output("date || credit || debit || balance\n13/01/2012 || || 800.00 || 200.00\n10/01/2012 || 1000.00 || || 1000.00\n").to_stdout
    end
  end
end
